sap.ui.define([
		...
		return {
			...
			getTermDate : function(date, term){
				
				var sDate = formatter.strToDate(date);
				var lDate = sDate.getDate();

				date = lDate + term;
				var toDate = new Date();
				toDate.setDate( date );
				
				return this.getDateStr(toDate); 
			},
...
//                ];
			...
			set_search_field : function(oWerks, Obj, ObjName, mode, param1, param2){
				...
				}else if(ObjName === "coc"){    // WHEN 'COC"
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + plant +"'"							
					}
					this._get_search_Model(Obj, "COC", urlParameters, mode, this.oMain.i18n.getText("lblKostl"), this.coc_fields);	

				}else if(ObjName === "woc"){    // WHEN 'WOC"
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + plant +"'"
					}
					this._get_search_Model(Obj, "WOC", urlParameters, mode, this.oMain.i18n.getText("lblKostl"), this.coc_fields);	

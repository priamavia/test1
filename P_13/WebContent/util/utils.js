sap.ui.define([
		...
		return {
			...
			makeSerachHelpHeader : function(oMain){
				...
				//Material Group 
				this.mag_fields = [
	               {label: this.oMain.i18n.getText("lblMaterialGroup"),     key: "Key", searchable:true, iskey:true, search:true },
				   {label: this.oMain.i18n.getText("lblMaterialGroupDesc"),  key: "Name", searchable:true, iskey:true, search:true }              
				];
				//MRP Controller
				this.mrc_fields = [
	                {label: this.oMain.i18n.getText("lblMRPController"),      key: "Key", searchable:true, iskey:true, search:true },
				    {label: this.oMain.i18n.getText("lblMRPcontrollerName"), key: "Name", searchable:true, iskey:true, search:true }
				];

				//Material Type
				this.mtp_fields = [
	                {label: this.oMain.i18n.getText("lblMaterialType"),      key: "Key", searchable:true, iskey:true, search:true },
				    {label: this.oMain.i18n.getText("lblMaterialTypeDesc"),  key: "Name", searchable:true, iskey:true, search:true }
				];
				
				//Material Sub Group
				this.sbg_fields = [
	                {label: this.oMain.i18n.getText("lblMaterialSubGroup"),      key: "Key", searchable:true, iskey:true, search:true },
				    {label: this.oMain.i18n.getText("lblMaterialGroupDesc"),  key: "Name", searchable:true, iskey:true, search:true }
				];				
...
//					var kokrs = param1;
				...
				}else if(ObjName === "mtp"){	// WHEN 'Mtp'.
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "'"
					}
					this._get_search_Model(Obj, "MTP", urlParameters, mode, this.oMain.i18n.getText("lblMaterialType"), this.mtp_fields);

				}else if(ObjName === "sbg"){	// WHEN 'SBG'.
					var mtart = param1;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '"+ mtart + "'"
					}
					this._get_search_Model(Obj, "SBG", urlParameters, mode, this.oMain.i18n.getText("lblMaterialSubGroup"), this.sbg_fields);
					  ...
					  if(objType === "C"){
						...
						if(key === "NOT"){			// WHEN 'NOT'. "1
							...
							controll._oMpc_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, true);  //Model, UI Object, SearchHelp table, title, MultiSelect 
						}else if(key === "MTP"){	// WHEN 'MTP'.
							controll._oMtp_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, true);  //Model, UI Object, SearchHelp table, title, MultiSelect 
						}else if(key === "SBG"){	// WHEN 'SBG'.
							controll._oSbg_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, true);  //Model, UI Object, SearchHelp table, title, MultiSelect 
						}							

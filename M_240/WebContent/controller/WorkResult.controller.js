sap.ui.define([
	...
	return Object.extend("cj.pm_m240.controller.WorkResult", {
		...
		set_header : function(sObj, sMode){
			...
			this.HistoryCalled = sObj.HistoryCalled;
			...
			if( (sObj.Stat == "I0009" || sObj.Stat == "I0045" || sObj.Stat == "I0046") 
					&& (sObj.Ustat != "E0004") ){
				if (sObj.HistoryCalled) {
					this.oMain.oController.getView().byId("resultCancel").setVisible(false);
				}else{
					this.oMain.oController.getView().byId("resultCancel").setVisible(true);
				}
				...
				if(this.sMode == "D" && sObj.Stat == "I0010" && sObj.Ustat == "E0003" ){
...
// 			this.bmcm_hwr = this.oMain.oController.getView().byId("bmcm_hwr");				
// 			this.act_hwr = this.oMain.oController.getView().byId("act_hwr");		 // ILART   			
			...
			if(this.sMode == "R"){
				...
				headerScreenModel.refresh(true);
			}
		},
			...
			if(sIdstr == "bmcm_hwr"){
				...
				//BM
				 if(key.getProperty("key") == "BM"){
					this.oMain.oController.getView().byId("qmcod_wr").setSelectedKey("01");
					oData.Mstatus = true;	
					oData.Msaus = true;
		...
		workResultList : function(){
			...
			var mParameters = {
					...
					success : function(oData) {
						...
						// work result 화면에서는 MeasuringList를 비활성화 한다. 
						for (var i=0; i<oData.MeasuringList.results.length; i++) {
							oData.MeasuringList.results[i].Enable = false;
						}

						if (oData.MeasuringList.results.length > 1) {
							oMdTable.setVisibleRowCount(oData.MeasuringList.results.length);
						}
						if (oData.WorktimeList.results.length > 1) {
							oWtTable.setVisibleRowCount(oData.WorktimeList.results.length);
						}
...
//						this.onMalfuncDuration();	
						...
						// 		&& (oData.Ustat != "E0004") ){
						// 	if (this.HistoryCalled) {
						// 		controll.cancel.setVisible(false);
						// 	}else{
						// 		controll.cancel.setVisible(true);	
						// 	}

						// 	controll.bmcm_hwr.setEditable(false);
						// 	controll.act_hwr.setEditable(false);
...
//			//PM Activity type 의 경우 Order type에 따라 달라져야 하므로 Order type 변경시 가져오도록 수정 
			if(this.sAuart){
				...
				if(this.oAct){
					utils.set_search_field(this.sWerks, this.oAct, "act", "C", this.sAuart, v_noti);
				}
			}				
		},
		...
		_getDialog_Catalog  : function(){
			...
			if (!this._oDialog_Catalog) {
				this._oDialog_Catalog = sap.ui.xmlfragment("cj.pm_m240.view.Catalog_pop", this);

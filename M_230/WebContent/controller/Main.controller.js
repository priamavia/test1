sap.ui.define([
		...
		return BaseController.extend("cj.pm_m230.controller.Main", {
			formatter: formatter,
...
//			debugger;
		...
		setInitData : function(){				
			...
			if(!this.oOrt.getSelectedItem()){
				utils.set_search_field("", this.oOrt, "ort", "C", "X", "");
			}else{
				this.oOrt.setSelectedKey(this.oOrt.getSelectedItem().getProperty("key"));
			}
			...
			check_data : function(){
				...
				var chk_outs = this.getView().byId("outstand").getSelected();
				var chk_inap = this.getView().byId("inappro").getSelected();
				var chk_inpr = this.getView().byId("inpro").getSelected(); 
				var chk_conf = this.getView().byId("conf").getSelected(); 
				var chk_comp = this.getView().byId("comp").getSelected(); 
	    	        		
	    		if(chk_outs === false && chk_inap === false && chk_inpr === false && 
				   chk_conf === false && chk_comp === false )	
	    		{
	    			sap.m.MessageBox.show(
...
*/
			...
			onSearch : function(){
				...
				if(arbpl){
			    	...
			    	if(s_arbpl){
			    		s_filter.push(utils.set_filter(s_arbpl, "Vaplz"));
				    }		
				}
				
		    	// Outstanding / In Approval / In-porcess / Completed / Rejected(delete at Tablet)
				var s_stat  = [];  
...
//			},
			...
			onItemPress : function(arg){
				...
				if(arg == 'M'){
					...
					if (obj.Equnr == "") {
						Message.show( "There is no equipment number.",
										 Message.Icon.ERROR,
										 this.i18n.getText("error") );	
						...
						if (arg == 'B') {
							//Display a Target Without Changing the Hash
							this._router.getTargets().display("bom", {
								fromTarget : "main",
								iEqunr  : obj.Equnr,
								iEqktx  : obj.Eqktx,
								iWerks  : obj.Werks
							...
							//Display a Target Without Changing the Hash
							this._router.getTargets().display("history", {
								fromTarget : "main",
								iEqunr  : obj.Equnr,
								iEqktx  : obj.Eqktx,
								iWerks  : obj.Werks
			...
			onSelectPress : function(oEvent){
				...
				// var idx = oEvent.getSource().getParent().getIndex();
				// var obj = this.getView().byId("listTable").getContextByIndex(idx).getObject();
				
				// this.get_order_data(obj.Aufnr, obj.Qmart);
				var oView = this.getView();				
				var oODataJSONModel =  new JSONModel();  
				oODataJSONModel.setData(obj);
			...
			get_order_data : function(aufnr, qmart){
				var lange = this.getLanguage();
				var oView = this.getView();
				...
				var path = "/HdSet(Mode='"+mode+"',Spras='"+lange+"',Aufnr='"+aufnr+"')";
...
//			},
	 ...
	 ****************************************************************/			
			...
			onPress_wkassign: function(){			// Request Approval Button, 반장/작업자가 승인자에게 요청
				...
				if(sObj){
					if(sObj.Zpm01){  // Internal Operation 
						...
						Message.show( "Internal Operation is not set.", Message.Icon.WARNING, this.i18n.getText("warning") );						
					}
				}
			},
			...
			onPress_wkresult: function(){
				// var idx = oEvent.getSource().getParent().getIndex();
				// var sObj = this.getView().byId("listTable").getContextByIndex(idx).getObject();
...
//
//		            this._oDialog_measureMemt = sap.ui.xmlfragment("cj.pm_m230.view.RecordMeasurement_pop", this);

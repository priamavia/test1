...
//					var kokrs = param1;
				...
				}else if(ObjName === "pur"){	// WHEN 'PUR'. 
					
					var ebeln = param1;
					var urlParameters = {
							"$expand" : "Result" ,
							"$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + ebeln + "'"	
					}
					this._get_search_Model(Obj, "PUR", urlParameters, mode, this.oMain.i18n.getText("lblPo"), "");	
			...
			openValueHelp : function(ObjName){
				...
				}else if(ObjName === "lfa" || ObjName === "lifnr"){
				...
				}else if(ObjName === "lfa_re" || ObjName === "lifnr_re"){
					this._oLfa_re_sh.openValueHelp(
							"/results", 
							function(selection,ctx){
						    },
						    function(ctx){
				...
				}else if(ObjName === "mat" || ObjName === "matnr" || ObjName === "matnr_re"){
		    ...
		    _get_search_Model : function(Obj, key, urlParameters, objType, title, field) {
				...
				var mParameters = {
				  ...
				  success : function(oData) {
					  ...
					  if(objType == "C"){				// Combobox
					  ...
					  }else if(objType == "A"){			// Array
			              for(var i=0; i<oData.Result.results.length; i++){
				                 Obj.push({Key: oData.Result.results[i].Key, KeyName: oData.Result.results[i].KeyName});
				          }
					  }else if(objType == "E"){			// Enter
			              for(var i=0; i<oData.Result.results.length; i++){
			            	  Obj = oData.Result.results[i];
				          }
			              
			              if(key == "MAT"){
			            	  this.oMain.setMaterial(Obj);
			              }else if(key == "PUR"){
			            	  this.oMain.setPoInfo(Obj);
			              }
						...
						if(key == "NOT"){			// WHEN 'NOT'. "1
							...
							if(Obj.getId() == "lifnr_re"){
								controll._oLfa_re_sh = new ValueHelpHelper(
				                		sh_Model, Obj, field, title, false);  //Model, UI Object, SearchHelp table, title, MultiSelect
								
								controll.openValueHelp(Obj.getId()); 

...
//                ];
			...
			set_search_field : function(oWerks, Obj, ObjName, mode, param1, param2){
				...
				}else if(ObjName === "cbc"){	// WHEN 'CBC'.
					var gsber = param1;
					var audit = param2;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + gsber +"'"
					}
					this._get_search_Model(Obj, "CBC", urlParameters, mode, "title", "field");
				}else if(ObjName === "aud"){	// WHEN 'AUD'. 
					var gsber = param1;
					var gubun = param2;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + gsber +"' and Input2 eq '" + gubun +"'"
					}
					this._get_search_Model(Obj, "AUD", urlParameters, mode, "title", "field");
				}else if(ObjName === "akl"){	// WHEN 'AKL'. 
					var gsber = param1;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + gsber +"'"
					}
					this._get_search_Model(Obj, "AKL", urlParameters, mode, "title", "field");
				}else if(ObjName === "slo"){	// WHEN 'SLO'. 
					var werks = param1;
					var gubun = param2;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + werks +"' and Input2 eq '" + gubun +"'"
					}
					this._get_search_Model(Obj, "SLO", urlParameters, mode, "title", "field");
				}else if(ObjName === "sbg"){	// WHEN 'SLO'. 
					var mtart = param1;
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Spras eq '"+ lange + "' and Input1 eq '" + mtart +"'"
					}
					this._get_search_Model(Obj, "SBG", urlParameters, mode, "title", "field");
				}

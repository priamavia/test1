sap.ui.define([
		...
		return {
			...
			makeSerachHelpHeader : function(oMain){
				...
				//Execution ID
				this.eid_fields = [
                    {label: this.oMain.i18n.getText("lblExecutionId"),      key: "Key", searchable:true, iskey:true, search:true },
				    {label: this.oMain.i18n.getText("lblExecutionTitle"), key: "Name", searchable:true, iskey:true, search:true }                        
                ];
			...
			set_search_field : function(oSwerk, Obj, ObjName, mode, param1, param2, openMode){
...
//					var kokrs = param1;
					...
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Input1 eq '" + plant + "' and Input2 eq '" + vaplz + "'"							
					}
				...
				}else if(ObjName === "wbs" || ObjName === "posid"){	// WHEN 'WBS'.
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Input1 eq '" + plant +"'"							
					}
					this._get_search_Model(Obj, "WBS", urlParameters, mode, this.oMain.i18n.getText("lblWbs"), this.wbs_fields, openMode);	
					
				}else if(ObjName === "eid" || ObjName === "exec_id" || ObjName === "t_exec_id" ){	// WHEN 'EID'.
					var urlParameters = {
							"$expand" : "Result" ,
						    "$filter" : "Input1 eq '" + plant +"'"							
					}
					this._get_search_Model(Obj, "EID", urlParameters, mode, this.oMain.i18n.getText("lblExecutionId"), this.eid_fields, openMode);	
			...
			openValueHelp : function(ObjName){
				...
				}else if(ObjName === "wbs" || ObjName === "posid"){
				...
				}else if(ObjName === "eid" || ObjName === "exec_id"){
					this._oEid_sh.openValueHelp(
						"/results", 
						function(selection,ctx){
					    },
					    function(ctx){
						    console.log("Eid_Error");
					    },
					    this
					);
				}else if(ObjName == "t_exec_id"){
					this._oT_Eid_sh.openValueHelp(
						"/results", 
						function(selection,ctx){
...
//			},
			...
			get_sh_help : function(obj){
				if(obj == "wbs"){
					...
					return this._oEid_sh;
				}else if(obj == "t_eid"){
					return this._oT_Eid_sh;	
				}
			},
			...
			/*	var cols = oTable.getColumns();
		       ...
		       */ 
		    _get_search_Model : function(Obj, key, urlParameters, objType, title, field, openMode, oEvent) {
...
//			              }			              
						...
						if(key == "NOT"){			// WHEN 'NOT'. "1
							...
							controll._oPic_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, true);  //Model, UI Object, SearchHelp table, title, MultiSelect 
						}else if(key == "GRD"){	// WHEN 'GRD'.  "30"
							controll._oGrd_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, true);  //Model, UI Object, SearchHelp table, title, MultiSelect 
						...
						}else if(key === "EID"){	// WHEN 'LFA'.  
							var strArr = Obj.getId().split("--");
							var cnt = strArr.length - 1;
							var sIdstr = strArr[cnt];
							
							if(sIdstr == "ExecId"){
								controll._oT_Eid_sh = new ValueHelpHelper(
				                	sh_Model, Obj, field, title, false);  //Model, UI Object, SearchHelp table, title, MultiSelect 
							}else{
								controll._oEid_sh = new ValueHelpHelper(
			                		sh_Model, Obj, field, title, false);  //Model, UI Object, SearchHelp table, title, MultiSelect 
							}
							if(openMode === "X"){    //Search Help를 open
								
                                if(sIdstr == "ExecId"){
                                	controll.openValueHelp("t_exec_id"); 
                                }else{
                                	controll.openValueHelp("eid");  // Vender 데이터의 경우 Help를 click 할때 데이터를 가져오고 help창은 제어 한다. 
                                }
							}
						}
...
//						}						
			...
			cal_time : function(startTime, endTime){
				var truncated;

sap.ui.define([
	...
	return Object.extend("cj.pm0110.util.ReqApproval", { 
		...
		workAssignList : function(sObj){
			...
			controll.selwerks = sObj.Werks; // 
			controll.selAufnr = sObj.Aufnr;
			controll.selZid   = sObj.Zid;   // PIC ???
			controll.selArbei = sObj.Arbei; //
			controll.selZdate = sObj.Addat; //
...
//		    //Header Info
			...
			for(var i=0; i<oTable.getSelectedIndices().length; i++){
				...
				item.Zplhr  = controll.selArbei.toString();
				...
				success : function(oData) {
				 ...
				 if(oData.RetType == "E"){
					...
					for(var i=0; i<oData.ResultList.results.length; i++){
					 ...
					 controll.oMain.clear_operation_pic();
				 }	 
